...
if ($_SESSION['usuario'] == 'admin' ) {
	$sql = "SELECT * FROM webs INNER JOIN usuarios ON webs.idUsuario = usuarios.idUsuario";
	$consulta = $conexion->prepare($sql);
	$consulta->execute();
	$webs = $consulta->fetchAll(PDO::FETCH_ASSOC);
}else{
	$sql = "SELECT dominio FROM webs WHERE idUsuario = :usuario ";
	$consulta = $conexion->prepare($sql);
...
if ($submit) {
	...
	$consulta = $conexion->prepare("SELECT idUsuario,dominio FROM webs WHERE dominio = :dominio ");
	$consulta->execute(array( 'dominio' => $_SESSION['usuario'].$_POST['dominio']));
	$usuario = $consulta->fetch(PDO::FETCH_ASSOC);

	if (!isset($usuario['dominio'])) {
		$consulta = $conexion->prepare("INSERT INTO `webs` (`idWeb`, `idUsuario`, `dominio`, `fechaCreacion`) VALUES (NULL, :idUsuario, :dominio, :date)");
